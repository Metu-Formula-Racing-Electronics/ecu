...
static void TSAL_task(void *args){
  ...
  while(1){
    if(bamocar.getBattVoltage() > 60){
      ...
      digitalWrite(TSAL_RED, !digitalRead(TSAL_RED)); //Toggle
    }else{
...
static void apps_task(void *args){
  int apps1 = 0;
  int apps2 = 0;
  int diff = 0;
  int correction = 0;
  while(1){
    ...
    if(diff > 10){
      apps_implausable++;
      if(apps_implausable > 2){//100ms of diff > 10
...
void setup() {
  ...
  xTaskCreate(&apps_task, "apps_task", 4096, NULL, 5, NULL);
